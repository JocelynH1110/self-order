...
// 目前 CartService 裡儲存所有項目的資料，其實只需要資料庫連結 *sql.DB，這樣就不用每次呼叫方法都要傳入 db 參數
...
func (c *CartService) AddItem(productID, quantity int) error {
...
func (c *CartService) RemoveItem(productID int) error {
	_, err := c.db.Exec("DELETE FROM cart_items WHERE product_id = ?", productID)
	return err
}

// ListProductsInCart 取得購物車中的商品資料
func (c *CartService) ListProductsInCart() ([]types.ListProductsInCartRow, error) {
	...
	var result []types.ListProductsInCartRow
	for rows.Next() {
		var i types.ListProductsInCartRow
